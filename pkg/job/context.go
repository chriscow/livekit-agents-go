...
func NewJobContext(parent context.Context) *JobContext {
	...
	return &JobContext{
		...
		shutdownOnce:  false,
		shutdownMu:    make(chan struct{}, 1), // buffered channel as mutex
	}
}
...
func (jc *JobContext) Shutdown(reason string) {
	// Try to acquire the shutdown mutex
	select {
	case jc.shutdownMu <- struct{}{}:
		// We got the mutex, proceed with shutdown
		defer func() { <-jc.shutdownMu }()
		...
		case <-time.After(5 * time.Second):
			slog.Warn("Shutdown hooks timed out after 5 seconds")
		}
	...
	default:
		// Another goroutine is already shutting down, just return
		return
	}
}
...
func (jc *JobContext) OnShutdown(callback func(reason string)) {
	...
	default:
		// If we can't get the mutex, job might be shutting down
		// Try again with a brief delay
		go func() {
			time.Sleep(10 * time.Millisecond)
			jc.OnShutdown(callback)
		}()
	}
}
